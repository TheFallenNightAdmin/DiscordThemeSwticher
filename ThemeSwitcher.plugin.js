...
module.exports = (_ => {
    const config = {
        info: {
            ...
            version: '2.0.0',
            description: 'A sleek remote theme switcher. Press Escape to open.',
        }
    };
    ...
    const THEMES = [
        ...
        { name: 'TIO',           color: '#CC0000', url: 'https://files.catbox.moe/pvnjal.txt'  },
        { name: 'Luna',          color: '#6A0DAD', url: 'https://files.catbox.moe/novg23.txt'  },
        { name: 'Blisma Corp',   color: '#f0b429', url: 'https://files.catbox.moe/u6vfjv.txt'  },
    ];
    ...
    return class ThemeSwitcher {
        ...
        start() {
            BdApi.DOM.addStyle(this._styleKey, CSS);
            this._buildPill();
            this._buildOverlay();
        ...
        stop() {
            ...
            if (this._activeNode) {
                try { this._activeNode.remove(); } catch (_) {}
                this._activeNode = null;
            }
            this._log('Stopped.');
        }
        ...
        async _applyTheme(name, silent = false) {
            ...
            try {
                ...
                this._err(`Failed to load "${name}":`, err);
                this._setRowStatus(name, 'error');
                BdApi.UI.showToast(`ThemeSwitcher: failed to load "${name}"`, { type: 'error' });
                return;
            }

            this._activeNode = BdApi.DOM.addStyle('ThemeSwitcher-theme', css);
        ...
        _fetchCSS(theme) {
            if (this._cssCache[theme.name]) return Promise.resolve(this._cssCache[theme.name]);
            return new Promise((resolve, reject) => {
                const xhr = new XMLHttpRequest();
                xhr.open('GET', theme.url, true);
                xhr.onload = () => {
                    if (xhr.status === 200) {
                        this._cssCache[theme.name] = xhr.responseText;
        ...
        _setRowStatus(name, state) {
            const row   = document.querySelector(`.ts-row[data-name="${CSS.escape(name)}"]`);
            const check = row?.querySelector('.ts-check');
            if (!check) return;
            if (state === 'loading') { check.textContent = '…'; check.style.color = '#666'; }
            else if (state === 'error') { check.textContent = '!'; check.style.color = '#f55'; }
